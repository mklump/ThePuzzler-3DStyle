...
namespace ns_DictionaryCreator
{
	...
	public class DictionaryCreator : Solution
	{
        ...
        /// <summary>
        /// This is a reference to the calling ASP.NET System.Web.UI.Page.
        /// </summary>
        public Page pageRef;
		/// <summary>
		...
		public RedBlack CreateDictionary( Page refPage, int baseDictionary, int size, int minimumWordLength )
		{
			minimumWordLength = (minimumWordLength < 3) ? 3 : minimumWordLength;
			GetDictionaryFile( refPage, baseDictionary );
            using( reader )
            {
                while( null != reader && false == reader.EndOfStream )
			    {
